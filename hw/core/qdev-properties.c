...
static void bit64_prop_set(Object *obj, const Property *props, bool val)
{
    ...
    if (props->offset == offsetof(VirtIONet, host_features)) {
        ...
        qemu_log("bit64_prop_set off %ld mask %lx features %lx ex " VIRTIO_FEATURES_FMT" \n",
                 props->offset, mask, n->host_features, VIRTIO_FEATURES_PRN_ARG(n->host_features_ex));
    }
}
...
static void vf_prop_set(Object *obj, const Property *props, bool val)
{
    ...
    if (props->offset == offsetof(VirtIONet, host_features_ex)) {
        ...
        qemu_log("bit64_prop_set off %ld bit %d features %lx ex " VIRTIO_FEATURES_FMT" \n",
                props->offset, props->bitnr, n->host_features, VIRTIO_FEATURES_PRN_ARG(n->host_features_ex));
    }
}

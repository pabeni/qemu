...
struct vhost_features {
    uint64_t count;
    uint64_t features[VIRTIO_FEATURES_WORDS / 2];
};
                                     ...
                                     virtio_features_t features)
{
    ...
    bool extended_in_use = false;
    int r, i;

    farray.count = VIRTIO_FEATURES_WORDS / 2;
    for (i = 0; i < VIRTIO_FEATURES_WORDS / 2; ++i) {
        farray.features[i] = features >> (64 * i);
        extended_in_use |= i > 0 && farray.features[i];
    }
                                     ...
                                     virtio_features_t *features)
{
    ...
    *features = 0;
    for (i = 0; i < VIRTIO_FEATURES_WORDS / 2; ++i) {
        virtio_features_t cur = farray.features[i];
